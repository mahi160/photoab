  ...
  try {
    const findMatch = () => {
      const activeCount = db
        .prepare("SELECT COUNT(*) as count FROM photos WHERE status = 'active'")
        .get().count;

      if (activeCount <= 1) {
        const winner = db
          .prepare(
            "SELECT * FROM photos WHERE status = 'active' ORDER BY wins DESC LIMIT 1",
          )
          .get();
      ...
      const rounds = db
        .prepare(
          "SELECT round, COUNT(*) as count FROM photos WHERE status = 'active' GROUP BY round ORDER BY round ASC",
        )
        .all();
      ...
      for (const r of rounds) {
        if (r.count >= 2) {
          const photos = db
            .prepare(
              "SELECT * FROM photos WHERE round = ? AND status = 'active' ORDER BY RANDOM() LIMIT 2",
            )
            .all(r.round);
          ...
          db.prepare(
            "UPDATE photos SET round = round + 1 WHERE round = ? AND status = 'active'",
          ).run(r.round);
          return "promoted";
        }
      }
    ...
    result
      ? res.json(result)
      : res.status(404).json({ error: "No duel found" });
  } catch (err) {
    console.error(err);
...
app.post("/api/vote", (req, res) => {
  ...
  try {
    const transaction = db.transaction(() => {
      db.prepare(
        "UPDATE photos SET round = round + 1, wins = wins + 1 WHERE id = ?",
      ).run(winnerId);
      db.prepare("UPDATE photos SET status = 'eliminated' WHERE id = ?").run(
        loserId,
      );
      db.prepare("INSERT INTO history (winner_id, loser_id) VALUES (?, ?)").run(
        winnerId,
        loserId,
      );
    });
...
app.post("/api/undo", (req, res) => {
  try {
    ...
    const transaction = db.transaction(() => {
      ...
      db.prepare(
        "UPDATE photos SET round = round - 1, wins = wins - 1 WHERE id = ?",
      ).run(lastAction.winner_id);
      // Revert Loser: Set status back to active
      db.prepare("UPDATE photos SET status = 'active' WHERE id = ?").run(
        lastAction.loser_id,
      );
  ...
  try {
    ...
    try {
      const cleanup = db.transaction(() => {
        ...
        db.prepare(
          "DELETE FROM sqlite_sequence WHERE name='photos' OR name='history'",
        ).run();
      });
      cleanup();
      // Optimize DB file
